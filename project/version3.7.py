...
class Game:
    ...
    def __init__(self):
        ...
        self.ship8_x, self.ship8_y = random.randint(self.SHIP_SIZE, self.WINDOW_WIDTH - self.SHIP_SIZE ), (
                    self.WINDOW_HEIGHT - 100)
        self.ship4_x, self.ship4_y = random.randint(self.SHIP_SIZE, self.WINDOW_WIDTH - self.SHIP_SIZE ), (
                    self.WINDOW_HEIGHT - 100)

        self.ship2_speed = 1.2
        self.ship1_speed = 1.0
        self.ship6_speed = 1.0
        self.ship8_speed = 1.0
        self.ship4_speed = 1.0
        self.score = 0
    ...
    def drawShamirCircle(self, width, cordinates):
        glPointSize(width)
    ...
    def draw_bomb(self):
        ...
        bombcordinates = midpoint_circle.MidpointCircleDrawing(self.bomb[0]+40,self.bomb[1],10)
        self.drawShamirCircle(2,bombcordinates)
    ...
    def ship4(self, midpoint_line, midpoint_cir):
        color_body = [0.8, 0.3, 0.2]
        color_window = [0.0, 1.0, 1.0]
        color_leg = [0.6, 0.6, 0.6]
        x_center, y_center = self.ship4_x + self.SHIP_SIZE // 2, self.ship4_y - self.SHIP_SIZE // 4
        radius_x, radius_y = self.SHIP_SIZE, self.SHIP_SIZE // 2  # Adjusted ship size

        #head
        self.draw_oval(x_center, y_center + 30, radius_x - 36, radius_y + 48, midpoint_line, color_window, 10, 170)
        midpoint_line.midpoint(x_center - 40, y_center + 42, x_center + 40, y_center + 42, color_window, 2)
        # Body
        midpoint_line.midpoint(x_center - 40, y_center + 42, x_center + 40, y_center + 42, color_body, 2)
        midpoint_line.midpoint(x_center - 40, y_center + 42, x_center - 70, y_center, color_body, 2)
        midpoint_line.midpoint(x_center + 40, y_center + 42, x_center + 70, y_center, color_body, 2)
        midpoint_line.midpoint(x_center - 70, y_center, x_center + 70, y_center, color_body, 2)
        midpoint_line.midpoint(x_center - 40, y_center, x_center - 40, y_center - 20, color_body, 2)
        midpoint_line.midpoint(x_center + 40, y_center, x_center + 40, y_center - 20, color_body, 2)
        midpoint_line.midpoint(x_center - 40, y_center, x_center + 70, y_center, color_body, 2)
        midpoint_line.midpoint(x_center - 36, y_center - 20, x_center + 36, y_center - 20, color_body, 2)
        # Leg
        midpoint_line.midpoint(x_center - 36, y_center - 28, x_center - 60, y_center - 70, color_leg, 2)
        midpoint_line.midpoint(x_center + 36, y_center - 28, x_center + 60, y_center - 70, color_leg, 2)
        midpoint_line.midpoint(x_center - 20, y_center - 24, x_center - 20, y_center - 64, color_leg, 2)
        midpoint_line.midpoint(x_center + 20, y_center - 24, x_center + 20, y_center - 64, color_leg, 2)
        midpoint_line.midpoint(x_center + 40, y_center, x_center + 90, y_center - 64, color_leg, 2)
        midpoint_line.midpoint(x_center - 40, y_center, x_center - 90, y_center - 50, color_leg, 2)
        midpoint_line.midpoint(x_center + 90, y_center - 64, x_center + 50, y_center - 100, color_leg, 2)
        midpoint_line.midpoint(x_center - 90, y_center - 50, x_center - 50, y_center - 100, color_leg, 2)

    def ship8(self, midpoint_line, midpoint_cir):
        color_body = [0.2, 1.0, 0.7]
        color_window = [1.0, 0.0, 1.0]
    
        x_center, y_center = self.ship8_x + self.SHIP_SIZE // 2, self.ship8_y - self.SHIP_SIZE // 4
        radius_x, radius_y = self.SHIP_SIZE, self.SHIP_SIZE // 2  # Adjusted ship size
        scaling_factor = 1.5  # Define the scaling factor to increase size (adjust as needed)

    # Wings
        self.draw_oval(x_center, y_center, radius_y, int(radius_y + 20 * scaling_factor), midpoint_line, color_window, 120, 240)
        self.draw_oval(x_center, y_center, int(radius_y + 10 * scaling_factor), int(radius_y + 40 * scaling_factor), midpoint_line, color_window, 120, 240)
        self.draw_oval(x_center, y_center, radius_y, int(radius_y + 20 * scaling_factor), midpoint_line, color_window, 300, 360)
        self.draw_oval(x_center, y_center, radius_y, int(radius_y + 20 * scaling_factor), midpoint_line, color_window, 0, 60)
        self.draw_oval(x_center, y_center, int(radius_y + 10 * scaling_factor), int(radius_y + 40 * scaling_factor), midpoint_line, color_window, 300, 360)
        self.draw_oval(x_center, y_center, int(radius_y + 10 * scaling_factor), int(radius_y + 40 * scaling_factor), midpoint_line, color_window, 0, 60)

    # Body
        midpoint_line.midpoint(x_center - int(10 * scaling_factor), y_center - int(60 * scaling_factor), x_center - int(10 * scaling_factor), y_center + int(60 * scaling_factor), color_body, 2)
        midpoint_line.midpoint(x_center + int(10 * scaling_factor), y_center - int(60 * scaling_factor), x_center + int(10 * scaling_factor), y_center + int(60 * scaling_factor), color_body, 2)
        midpoint_line.midpoint(x_center - int(10 * scaling_factor), y_center + int(60 * scaling_factor), x_center + int(10 * scaling_factor), y_center + int(60 * scaling_factor), color_body, 2)

    # Head
        midpoint_line.midpoint(x_center - int(10 * scaling_factor), y_center - int(60 * scaling_factor), x_center, y_center - int(80 * scaling_factor), color_body, 2)
        midpoint_line.midpoint(x_center, y_center - int(80 * scaling_factor), x_center + int(10 * scaling_factor), y_center - int(60 * scaling_factor), color_body, 2)

        midpoint_line.midpoint(x_center - int(40 * scaling_factor), y_center + int(10 * scaling_factor), x_center - int(10 * scaling_factor), y_center, color_body, 2)

        midpoint_line.midpoint(x_center - int(10 * scaling_factor), y_center, x_center - int(40 * scaling_factor), y_center - int(10 * scaling_factor), color_body, 2)
    ...
    def display(self):
        ...
        for bullet in self.bullets:
            ...
            if self.has_collided([bullet[0], bullet[1], 0, 0],
                ...
                if self.hit_count_ship3 == 500: 
                    self.score += 10 
                    self.boss = False
                    
                    self.hit_count_ship3 = 0
                    self.ship3_y = (self.WINDOW_HEIGHT - 20)
                    self.ship3_x = random.randint(1, self.WINDOW_WIDTH - self.SHIP_SIZE)

#########################ship-4
#             if self.has_collided([bullet[0], bullet[1], 0, 0],
#                                 [self.ship4_x, self.ship4_y - self.SHIP_SIZE, self.SHIP_SIZE, self.SHIP_SIZE]):
                
#                 self.hit_count_ship4 += 1
                

#                 if self.hit_count_ship4 == 20:
#                     self.score += 1
#                     print("Score:", self.score)
#                     self.hit_count_ship4 = 0
#                     self.ship4_y = (self.WINDOW_HEIGHT - 20)
#                     self.ship4_x = random.randint(1, self.WINDOW_WIDTH - self.SHIP_SIZE)
#                     self.ship4_speed += 0.2
...
# ############################ship-8
#             if self.has_collided([bullet[0], bullet[1], 0, 0],
#                                 [self.ship8_x, self.ship8_y - self.SHIP_SIZE, self.SHIP_SIZE, self.SHIP_SIZE]):
                
#                 self.hit_count_ship8 += 1
                

#                 if self.hit_count_ship8 == 20:
#                     self.score += 1
#                     print("Score:", self.score)
#                     self.hit_count_ship8 = 0
#                     self.ship8_y = (self.WINDOW_HEIGHT - 20)
#                     self.ship8_x = random.randint(1, self.WINDOW_WIDTH - self.SHIP_SIZE)
#                     self.ship8_speed += 0.2



                     
                                
            # if ((self.ship1_y+self.SHIP_SIZE ) <= 0) :
            #     self.ship1_y = (self.WINDOW_HEIGHT - 20)
            #     self.ship1_x = random.randint(1, self.WINDOW_WIDTH - self.SHIP_SIZE)
            #     self.ship1_speed += 0.1

            # if ((self.ship2_y+self.SHIP_SIZE ) <= 0) :
            #         if ((self.WINDOW_WIDTH//2)-50 <= self.ship1_x <= (self.WINDOW_WIDTH//2)+50) or (self.ship1_x == (self.WINDOW_WIDTH//2 )):
            #             self.ship2_x  = random.choice([self.SHIP_SIZE+10, self.WINDOW_WIDTH-(self.SHIP_SIZE+10)])
            #         else:
            #             if self.ship1_x< (self.WINDOW_WIDTH//2):
        ...
        if not self.game_over and not self.paused:
            ...
            if self.collision==3:
                ...
                ax = 42
                ay = 16
                if (self.is_point_inside(x_center-ax,y_center-ay,".") or self.is_point_inside(x_center+ax,y_center-ay,".") or self.is_point_inside(x_center,y_center-ax,".") or self.is_point_inside(x1_center-ax,y1_center-ay,".") or self.is_point_inside(x1_center+ax,y1_center-ay,".") or self.is_point_inside(x1_center,y1_center-ay,".")):
                    self.collision+=1
            ...
            # Updating the x cordinate of bomb
        ...
        # if self.score >=30: 
        #     self.ship8(midpoint_line,midpoint_cir)
        # #if self.score >=50: 
        #     self.ship4(midpoint_line,midpoint_cir)
